<script lang="ts">
  import { teams, teamRosters, allCyclists } from "../stores";
  ...
  $: { const m: Record<string,number>={};
    roster.forEach(r=>{m[r.rider_type]=(m[r.rider_type]??0)+1;});
    topType = Object.entries(m).sort((a,b)=>b[1]-a[1])[0]?.[0]??"-"; }
  let topType = "-";

  function sel(t: Team) { selected = t; }
</script>
...
<div class="teams section-enter">
    ...
    <div class="list-scroll">
      {#each filtered as t}
        <button class="team-btn" class:active={selected?.id===t.id} on:click={()=>sel(t)}>
          <span class="t-flag">{flagEmoji(t.country_iso)}</span>
  ...
  <div class="detail">
    ...
    {:else}
      <div class="team-hdr">
        <span class="t-flag2">{flagEmoji(selected.country_iso)}</span>
        <span class="t-hname">{selected.name}</span>
...
<style>
  ...
  .t-short { font-size:13px; color:#6478a0; }
  .stat-cards { display:grid; grid-template-columns:repeat(5,1fr); gap:10px; flex-shrink:0; }
  .roster-table { flex:1; overflow:hidden; background:#0f1829; border:1px solid #1e2d4a; border-radius:8px; }
</style>
